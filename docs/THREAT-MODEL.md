...
- **Retention is implemented, and what it does not bound is the audit records.** An
  event's content is deleted fourteen days after the event ends, with a notice from day 7
  (ADR-0013, built 2026-09-23). ADR-0004 accepts its residual partly on the strength of a
  small local store, and that is now a property rather than an assumption. Audit records
  still grow without bound, by design, so the store does not shrink to nothing when events
  are deleted. `fieldnote-tcq` and `fieldnote-iox`, both closed; ADR-0004 as amended
  2026-09-11 and 2026-09-23; ADR-0008.
